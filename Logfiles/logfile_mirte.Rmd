...
```{r}
model <- mdatools::plsda(x = t(rna_seq_data[,-1]), c = group2, cv = 1)
```
...
plot(model)
```
VOOR DE PLOTS ENZO NOG EVEN KIJKEN HOE DIT PRECIES ZIT.

```{r}
mdatools::getConfusionMatrix(model$calres)
```
Kijkend naar de confusion matrix lijkt er nog iets fout te gaan?
