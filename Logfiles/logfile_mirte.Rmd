...
top_genes <- lapply(1:7, function(comp) {
  ...
  sort_genes <- names(sort(abs(loadings_comp), decreasing = TRUE))[1:3]
...
Now I have a dataframe containing the top 3 genes that contributed the most to each component. I will take a look at the top 3 of each component. 
...
Looking at the top 3 genes for each component each top 3 is completely unique. Each component is seperated by completely unique genes. To make sure whether or not the seperations make biological sense, I will have to look up the functions of the genes.

**26-5-2025:**

I will start with looking up the functions of the top 3 of component 1.

| Gene:  | Function:                                                                                                                                                                                                                                                                                                                                                |
|--------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Calu   | Calcium binding protein localized in the endoplasmic reticulum (ER). Involved in ER functions.                                                                                                                                                                                                                                                           |
| Washc2 | Wasc2CaluEnables phsophatidylinositol phosphate binding activity, phosphatidylinositol-3,4-bisphosphate binding activity, and retromer complex binding activity. Involved in endosomal transport, negative regulation of barbed-end actin filament capping, protein localization to the endosome. Located in the cytosol, early endosome, and nucleolus. |
| Sun1   | Member of the unc-84 homolog family. Encodes a nuclear envelope protein with an Unc84 domain. Involved in nuclear Anchorage and migration.                                                                                                                                                                                                               |

Component 2:

| Gene:  | Function:                                                                                                                                                                                    |
|--------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Kpna2  | Imports proteins into the nucleulus, a process that involves at least 2 steps. Plays a role in V(D)J recombination as well.                                                                  |
| Atp1b1 | The protein belongs to the Na+/K+ and H+/K+ ATPases beta chain proteins family. Is integral for membrane protein responisble for establishing and maintaining the electrochemical gradients. |
| Gpd1   | Encodes a member of the NAD-dependent glycerol-3-phosphate dehydrogenase family. Plays a critical role in carbohydrate and lipid metabolism.                                                 |

Component 3:

| Gene: | Function:                                                                                                                                                                                                                                                                                                   |
|-------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Saa3  | Enables toll-like receptor 4 binding activity and chemoattractant activity.                                                                                                                                                                                                                                 |
| Slpi  | Encodes a secreted inhibitor which protects ephithelial tissues from serine proteases. Found in various secretions. Contributes to the immune response by protecting epithelial surfaces from attack by endogenous proteolytic enzymes.                                                                     |
| Lcn2  | Encodes a protein that belongs to the lipocalin family. Members of this family transport small hydrophobic molecules. The presence of this protein in blood and urine is an early biomarker of acute kidney injury. Involved in maintenance of skin homeostasis, supression of invasiveness and metastatis. |

Component 4:

| Gene:    | Function:                                                                     |
|----------|-------------------------------------------------------------------------------|
| Gm2991   | Predicted pseudogene, function unclear                                        |
| Tma7-ps  | Associated with translational machinery.                                      |
| Rps2-ps6 | Pseudogene most likely associated with ribosomes. Catalyze protein synthesis. |

Component 5:

| Gene:     | Function:                          |
|-----------|------------------------------------|
| Gm13436   | No functions found.                |
| Gm10184   | Predicted pseudogene.              |
| Hmgb1-ps7 | High mobility group, pseudogene 7. |

Component 6:

| Gene:     | Function:                            |
|-----------|--------------------------------------|
| Gm14681   | No functions found.                  |
| Rpl17-ps3 | Ribosomal protein L17, pseudogene 3. |
| Gm10269   | Pseudogene, no function found.       |

Component 7:

| Gene:    | Function:                                                                                                                                            |
|----------|------------------------------------------------------------------------------------------------------------------------------------------------------|
| Sftpc    | Encodes the pulmonary-associated surfactant protein C (SPC), an extremely hydrophobic surfactant protein essential for lung cuntion and homeostasis. |
| Rpsa-ps7 | Pseudogene similiar to 40S ribosomal protein.                                                                                                        |
| Gm29216  | Pseudogene, no function found.                                                                                                                       |

All information about the genes was derived from https://www.genecards.org/ and from https://www.informatics.jax.org/

When looking at the tables I made containing the information about each gene of the top 3 of each component there are 2 that are particullary intresting: Slpi and Sftpc. Slpi encodes a secreted inhibitor which protects ephithelial tissues from serine proteases, this happens in the lungs as well. So this is something we can connect to the original article. Sftpc encodes the pulmonary-associated surfactant protein C (SPC), an extremely hydrophobic surfactant protein essential for lung funtion and homeostasis, pulmonary-associated surfactant protein C is present i n the lungs. So this is something we can connect to the original article as well.
...
Today I wrote the materials and methods for the PLS-DA.
...
Today I added the PLS-DA results to our paper.
