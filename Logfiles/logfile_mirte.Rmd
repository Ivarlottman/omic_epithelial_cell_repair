...
Today I am going to start working on implementing the PLS-DA, for this I will ask my teacher for help because I have already searched some information but I am having a hard time figuring it out myself. After getting some help from my teacher I have a workings PLSDA.
...
```{r}
model <- mdatools::plsda(x = t(rna_seq_data[,-1]), c = group2, ncomp = 4, scale = TRUE, cv = 1)
```
...
```{r}
summary(model, ncomp = 4)
```

Besides the summary I can generate a confusion matrix as well. The confusion matrix shows as which class everything was classified. Something remarkable is the fact that most of the samples were classed as ```None```. All of the ```ctrl_CCL``` was classed correctly, 3 of the ```EVs_CCL``` were classed correctly, and 3 of the ```OGN_CCL``` were classed correctly. So only samples from the CCL cell line were classed correctly, none of the samples from the Epcam were classed correctly.

```{r}
mdatools::getConfusionMatrix(model$calres, ncomp = 4)
```
Another way to show the results is plotting the predictions this is similiar to the confusion matrix.

```{r}
mdatools::plotPredictions(model, ncomp=4)
```
